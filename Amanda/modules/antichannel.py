...
@pbot.on_message(filters.command(["banchnl"]) & filters.group)
...
@pbot.on_message(filters.command(["unbanchnl"]) & filters.group)
...
@pbot.on_message(filters.command(["addwhitelistchnl"]) & filters.group)
...
@pbot.on_message(filters.command(["delwhitelistchnl"]) & filters.group)
...
@pbot.on_message(filters.command(["showwhitelistchnl"]) & filters.group)
async def del_whitelist_handler(bot, message):
    chat_id = message.chat.id
    user = await bot.get_chat_member(chat_id, message.from_user.id)
    if user.status == "creator" or user.status == "administrator":
...
__help__ = """
/banchnl [channel_id] : ban channel from sending message as channel.
/unbanchnl [channel_id] : unban channel from sending message as channel.
/addwhitelistchnl [channel_id] : add channel into whitelist and protect channel for automatic actions.
/delwhitelistchnl [channel_id] : remove channel from whitelist.
/showwhitelistchnl : Show all white list channels.
"""

BANCHNL = DisableAbleCommandHandler("approve", approve)
UNBANCHNL = DisableAbleCommandHandler("unapprove", disapprove)
ADDWHITELISTCHNL = DisableAbleCommandHandler("approved", approved)
DELWHITELISTCHNL = DisableAbleCommandHandler("approval", approval)
SHOWWHITELISTCHNL = DisableAbleCommandHandler("unapproveall", unapproveall)

dispatcher.add_handler(BANCHNL)
dispatcher.add_handler(UNBANCHNL)
dispatcher.add_handler(ADDWHITELISTCHNL)
dispatcher.add_handler(DELWHITELISTCHNL)
dispatcher.add_handler(SHOWWHITELISTCHNL)

__mod_name__ = "Anti-CHNL"        
